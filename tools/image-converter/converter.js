...
(function () {
  ...
  var GEMINI_PROMPT = [
    ...
    'Your task is to determine the CAMERA POSITION relative to the product.',
    'Look at which surface of the product takes up the MOST area in the image.',
    'Respond with EXACTLY one category name (nothing else, no explanation):',
    '',
    'voorkant - Camera is directly in front of the product, straight-on. Only the front surface is visible.',
    'vooraanzicht - Camera is mostly in front but slightly offset/angled, so the front is dominant but a second surface (top or side) is also partially visible.',
    'zijkant - Camera is directly to the side of the product, straight-on. Only one side surface is visible.',
    'zijaanzicht - Camera is mostly to the side but slightly angled, so the side is dominant but another surface is also partially visible.',
    'achterkant - Camera is directly behind the product, straight-on. Only the rear surface is visible (often shows ports, connectors, labels).',
    'achterzijde - Camera is mostly behind but slightly angled, so the rear is dominant but another surface is also partially visible.',
    'bovenkant - Camera is directly ABOVE the product, looking straight down (bird\'s eye / top-down view). The top surface takes up most of the image.',
    'bovenzijde - Camera is mostly above but at an angle, so the top surface is dominant but the front or side is also partially visible.',
    'onderkant - Camera is directly BELOW the product, looking straight up. The bottom surface takes up most of the image.',
    'onderzijde - Camera is mostly below but at an angle, so the bottom is dominant but another surface is also visible.',
    'detail - A close-up/macro shot focusing on one specific small feature (a port, button, label, texture) rather than the whole product.',
    'afstandsbediening - The main subject of the photo is a remote control (a separate handheld accessory with buttons, NOT the main product itself).',
    'scenario - The product is shown in use in a real environment (mounted, installed, on a desk, in a room — lifestyle/context photo).',
    '',
    'CRITICAL RULES:',
    '- Focus on which SURFACE of the product occupies the most image area, not on individual features like lenses or logos.',
    '- If you can see mostly the top of a product (vents, top panel), that is bovenkant/bovenzijde even if the lens or front edge is slightly visible.',
    '- "-kant" = perfectly flat/straight-on single surface view. "-aanzicht"/"-zijde" = same dominant surface but with perspective showing depth.',
    '- "afstandsbediening" is ONLY for standalone remote controls as the main subject.',
    '',
